# @webabcd/harmony-httpserver 的 demo
...
| handleHttpRequest | 处理指定的请求，并同步响应数据 |
| handleHttpRequestAsync | 处理指定的请求，并异步响应数据 |
| enableLog | 是否打印日志 |

## 示例代码（本地 http server 收到请求后，同步指定响应数据）
```
...
## 示例代码（本地 http server 收到请求后，异步指定响应数据）
```
import { HttpRequest, HttpResponse, httpServer } from '@webabcd/harmony-httpserver'
import { BusinessError } from '@kit.BasicServicesKit';
import { http } from '@kit.NetworkKit';
